...
const NickName = () => {
 ...
 const { data: getBio, isLoading: bioLoader } = useQuery({
    queryKey: ["nicknames", page],  // Include page in the query key
    queryFn: () => getData(token, `nickname?page=${page}`),  // Modify endpoint to accept page parameter
  });
  ...
  const mutation = useMutation({
    mutationFn: addData,
    onSuccess: async () => {
      queryClient.invalidateQueries(["nicknames"]);
      notify(" Created");
  ...
  return (
    <div>
      ...
      <div className="flex flex-row justify-between mt-10">
        <h2 className="text-2xl">All the nickname</h2>
